...
const CustomButton: React.FC<CustomButtonProps> = ({
  ...
  const baseClassName =
    variant === "primary"
      ? `inline-block ${sizeClasses} bg-gray-100 text-gray-800 border-2 border-gray-800 font-serif font-semibold transition-all duration-300 ease-in-out hover:bg-gray-200 hover:shadow-md relative overflow-hidden group`
      : `inline-block ${sizeClasses} bg-white text-gray-700 border border-gray-300 font-serif font-semibold relative overflow-hidden group hover:shadow-md hover:bg-gray-50`
  ...
  const content =
    variant === "primary" ? (
      <>
        ...
        <span className="absolute inset-0 bg-gray-200 transform -skew-x-12 -translate-x-full transition-transform duration-300 ease-out group-hover:translate-x-0"></span>
      </>
    ) : (
      <>
        <span className="relative z-10">{text}</span>
        <span className="hidden absolute inset-0 bg-gray-100 transform scale-x-0 origin-left transition-transform duration-300 ease-out group-hover:scale-x-100"></span>
      </>
    )
